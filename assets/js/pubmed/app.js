$(document).ready(function () {
    ...
    $(document).on('click', '#report_add_btn', function () {
        ...
        $.ajax({
            url: base_url + 'pubmed/admin_api/report_add',
            type: 'post',
            data: {
                'title': $('#title').val(),
                'conditions': $('#term').val(),
    ...
    function updateReport(report_id) {
        ...
        $.ajax({
            url: base_url + 'pubmed/admin_api/report_update',
            type: 'post',
            data: {
                'id': report_id,
    ...
    function searchReport(keyword, sort) {
        $.ajax({
            url: base_url + 'pubmed/admin_api/report_search',
            type: 'post',
            data: {
                keyword: keyword,
                sort: sort
            ...
            success: function (resp) {
                resp = JSON.parse(resp);
                if (resp.success) {
                    $('#dashboard_pubmed #report_list').html(resp.reports);
                }
                else {
                    $('#dashboard_pubmed #report_list').html('');
                }
            }
        })
    }

    $(document).on('keypress', '#dashboard_pubmed #report_search_input', function (event) {
        var keycode = (event.keyCode ? event.keyCode : event.which);
        if (keycode == '13') {
            searchReport($(this).val(), $('#dashboard_pubmed #sort').val())
        }
    })

    $(document).on('click', '#dashboard_pubmed .search-input-icon', function () {
        searchReport($("#dashboard_pubmed #report_search_input").val(), $('#dashboard_pubmed #sort').val());
    })

    // sort
    $(document).on('change', '#dashboard_pubmed #sort', function () {
        searchReport($('#dashboard_pubmed #report_search_input').val(), $(this).val());
    })
    ...
    $(document).on('click', '.report-list-action-popup-btn--reporting', function () {
        ...
        $(report_row).addClass('loading');
        $.ajax({
            url: base_url + 'pubmed/admin_api/report_reporting',
            type: 'post',
            data: {
                'id': report_id,
                reporting: reporting
    ...
    $(document).on('click', '.report-list-action-popup-btn--duplicate', function () {
        ...
        $(report_row).addClass('loading');
        $.ajax({
            url: base_url + 'pubmed/admin_api/report_duplicate',
            type: 'post',
            data: {
                'id': report_id,
            },
    ...
    $(document).on('click', '.report-list-action-popup-btn--delete', function () {
        ...
        showConfirm({
            ...
            $(report_row).addClass('loading');
            $.ajax({
                url: base_url + 'pubmed/admin_api/report_delete',
                type: 'post',
                data: {
                    'id': report_id,
                },
    ...
    $(document).on('click', '#dashboard_pubmed .report-list-download-btn', function () {
        var report_id = $(this).parents('.report-list-row').attr('report-id');
        window.open(base_url + 'pubmed/admin_api/rss_download?report_id=' + report_id, '_blank');
    })

    $(document).on('click', '#dashboard_pubmed #title', function (event) {
    ...
    $(document).on('click', '#dashboard_pubmed .report-add-area', function (event) {
    ...
    $(document).on('click', '.btn-user-delete', function () {
        ...
        showConfirm({
            ...
            $.ajax({
                url: base_url + 'pubmed/admin_api/user_delete',
                type: 'post',
                data: {
                    'user_id': user_id,
                },
    ...
    $(document).on("click", "#dashboard_pubmed .show_line_btn", function () {
        ...
        $("#dashboard_pubmed .date_picker_wrap").attr("report-id", report_id);
        $("#dashboard_pubmed .report_date-downlaod").attr("report-id", report_id);

        $("#dashboard_pubmed #start_date_pubmed").val("");
        $("#dashboard_pubmed #last_date_pubmed").val("");

        $.ajax({
            url: base_url + 'pubmed/admin_api/report_get_week_list',
            type: 'post',
            data: {
                'id': report_id,
            },
            success: function (resp) {
                ...
                if (resp.week_list == null || resp.week_list == "") {
                    ...
                    $('#dashboard_pubmed .graph-songs').graphiq({
                        data: week_val_obj,
                        fluidParent: ".col",
                        height: 100,
                        xLineCount: 10,
                    ...
                    */
                    ...
                    $('#dashboard_pubmed .graph-songs_total').graphiq({
                        data: week_val_obj,
                        fluidParent: ".col",
                        height: 200,
                        xLineCount: 10,
                ...
                $("#dashboard_pubmed .modal-title").text(title);
        ...
        //remove
        $("#dashboard_pubmed .graph-songs").empty();
        $("#dashboard_pubmed .graph-songs_total").empty();


        $("#dashboard_pubmed .show_modal_btn").trigger("click");
    });
    ...
    //date pikcer
    $("#dashboard_pubmed #start_date_pubmed").datepicker({
        dateFormat: 'mm / dd / yy',
        maxDate: 0,
        onSelect: function (dateText) {
            ...
            var last_difference_in_day = $("#dashboard_pubmed #last_date_pubmed").val();
            if (last_difference_in_day == "") {
                $('#dashboard_pubmed #last_date_pubmed').datepicker("setDate", new Date(today));
                last_difference_in_day = $("#dashboard_pubmed #last_date_pubmed").val();
            }
            ...
            $('#dashboard_pubmed #last_date_pubmed').datepicker("option", "minDate", selected_date);

          
            var report_id = $("#dashboard_pubmed .date_picker_wrap").attr("report-id");
    ...
    $("#dashboard_pubmed #last_date_pubmed").datepicker(
        {
            dateFormat: 'mm / dd / yy',
            maxDate: 0,
            onSelect: function (dateText) {
                ...
                var last_difference_in_day = $("#dashboard_pubmed #start_date_pubmed").val();
                
                if (last_difference_in_day == "") {
                    $('#dashboard_pubmed #start_date_pubmed').datepicker("setDate", new Date(selected_date));
                    last_difference_in_day = $("#dashboard_pubmed #start_date_pubmed").val();
                }
                ...
                $('#dashboard_pubmed #start_date_pubmed').datepicker("option", "maxDate", selected_date);
    ...
    $(document).on("click", "#dashboard_pubmed .set_third_days_btn", function () {

        var today = new Date();
        var priorDate = new Date(new Date().setDate(today.getDate() - 30));

        $('#dashboard_pubmed #start_date_pubmed').datepicker("setDate", new Date(priorDate));
        $('#dashboard_pubmed #last_date_pubmed').datepicker("setDate", new Date(today));

        $('#dashboard_pubmed #start_date_pubmed').datepicker("option", "maxDate", today);
        $('#dashboard_pubmed #last_date_pubmed').datepicker("option", "maxDate", today);
    ...
    $(document).on("click", "#dashboard_pubmed .change_display_cumulative", function () {
        $("#dashboard_pubmed .graph-songs_wrap").css("display", "none");
        $("#dashboard_pubmed .graph-songs_total_wrap").css("display", "block");

        if ($("#dashboard_pubmed .change_display_daily").hasClass('active')) {
            $("#dashboard_pubmed .change_display_daily").removeClass('active');
            $("#dashboard_pubmed .change_display_cumulative").addClass('active');
        }

    });

    $(document).on("click", "#dashboard_pubmed .change_display_daily", function () {
        $("#dashboard_pubmed .graph-songs_wrap").css("display", "block");
        $("#dashboard_pubmed .graph-songs_total_wrap").css("display", "none");

        if ($("#dashboard_pubmed .change_display_cumulative").hasClass('active')) {
            $("#dashboard_pubmed .change_display_cumulative").removeClass('active');
            $("#dashboard_pubmed .change_display_daily").addClass('active');
        }
    });
    ...
    $(document).on("click", "#dashboard_pubmed .report-list-col-status", function () {
        ...
        $("#dashboard_pubmed .popup-title").text(name_title);


        $.ajax({
            url: base_url + 'pubmed/admin_api/popup_update',
            type: 'post',
            data: {
                'id': report_id,
            },
            success: function (resp) {
                ...
                $("#dashboard_pubmed .popup_body").html(tbody_string);
                ...
                $("#dashboard_pubmed .popup_updates_count").text(update_numbers);

            }
        })

        $("#dashboard_pubmed .show_popup_btn").trigger("click");

    })


    $(document).on("click", "#dashboard_pubmed .download_list_report_btn", function () {


        var report_id = $(this).parents('.report-list-row').attr('report-id');
        window.open(base_url + 'pubmed/admin_api/download_csv?report_id=' + report_id, '_blank');
    })


    //download dates csv
    $(document).on("click", "#dashboard_pubmed .report_date-downlaod", function () {

        var report_id = $(this).attr('report-id');

        var start = $("#dashboard_pubmed #start_date_pubmed").val();
        var last = $("#dashboard_pubmed #last_date_pubmed").val();
        ...
        if (start == "") {
            ...
            window.open(base_url + 'pubmed/admin_api/download_dates_csv?report_id=' + report_id +'&start_date=' + start_difference +'&last_date=' + last_difference +'&last_date_day=' + last_date_day + '&start_date_day=' + start_date_day, '_blank');
        }
